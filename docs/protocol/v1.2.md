...
* `datasets.charts` 中 `pie` / `doughnut` 图的基础结构
...
当前版本中，`datasets.charts` 正式定义两种图表类型：
...
| `label`     | string | 否       | 人类可读标签     | 普通字符串                            |
| `meta`      | object | 否       | 附加元信息       | 当前不商定内部字段                    |
| `chartType` | string | 是       | 图表类型         | 枚举值：`pie` 饼图，`doughnut` 圆环图 |
...
16. `datasets.charts` 中 `pie.slices` / `doughnut.slices` 结构合法
17. `doughnut.total > 0`
18. `doughnut.slices[].value >= 0`
19. `sum(doughnut.slices[].value) <= doughnut.total`
20. `citation_ref.targetId` 可解析到 `bibliography[].id`
21. `footnote_ref.targetId` 可解析到 `footnotes[].id`
22. `glossary_term.targetId` 可解析到 `glossary[].id`
23. `theme.blockStyleDefaults` 的结构
24. `block.layout` 的字段与枚举
25. `table.layout.columnSpecs`
26. `width.mode = auto|weight`
27. `weight.value > 0`
28. `grid.cells[].blocks` 仅允许 `_type="block"` / `_type="image"` / `_type="chart"`
29. `sectionRole` 枚举合法
30. `navigation.includeInToc` 为 boolean
31. `pagination.pageBreakBefore` 枚举合法
32. `presentation` 结构合法

---

## 18. 渲染器最低实现要求

为保证 v1.2 能真正落地，渲染器至少应实现以下能力：
...
10. 支持 `datasets.charts` 中 `pie` / `doughnut`
...
16. 支持 `sectionRole / navigation / pagination / presentation`
17. 支持 `table.layout.columnSpecs[]`
18. 支持 `width.mode = auto|weight`
19. 支持单元格内 `_type="image"` / `_type="chart"`
20. 单元格图片与图表默认适配可用宽度并保持可辨识布局
21. 至少支持 `navigation.includeInToc = false` 对 TOC 生成的影响
22. 至少支持 `pagination.pageBreakBefore = "always"` 对 section 起始前分页的影响
...
```json
{
  "schemaVersion": "report.v1.2",
  "meta": {
    "title": "OVA Portable Text v1.2 Minimal Example",
    "language": "zh",
          ...
          "blocks": [
            ...
            },
            {
              ...
              "children": [
                {
                  "_type": "span",
                  "text": "表 1. v1.2 grid 表格、列宽权重、单元格图表与图片示例。",
                  "marks": []
                }
              ],
              "markDefs": []
