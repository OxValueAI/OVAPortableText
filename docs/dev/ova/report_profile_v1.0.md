...
| `label`       | string  | 否       | 人类可读标签     | 普通字符串                       |
| `meta`        | object  | 否       | 附加元信息       | 当前不冻结内部字段               |
| `type`        | string  | 是       | 文献类型         | 见 11.4                          |
| `title`       | string  | 是       | 文献标题         | 普通字符串                       |
...
| `text`        | string  | 否       | 单字段兼容文案   | 兼容字段，不建议作为正式主要结构 |
...
* 根据 `citation_ref.targetId` 解析到 `bibliography[].id`
* 将参考文献条目渲染为可读文本
* 当字段不完整时进行降级展示
...
1. 若其他字段缺失，不应导致文献列表失败
2. 若 `targetId` 无法解析，渲染器可以显示占位引用并记录日志
...
* 根据 `glossary_term.targetId` 解析到 `glossary[].id`
* 读取并展示：
* `term`
* `definition`
...
1. 若 `aliases` 存在，渲染器可以用于搜索、匹配或索引，但不是必须展示
2. 若 `definition` 缺失，应视为条目无效
3. 若 `targetId` 无法解析，渲染器可以按普通文本降级处理并记录日志
...
3. 顶层 `sections[]` 一般承载一级章节，但当前版本不强制只能是 `level = 1`
4. 渲染器不得自行重排 `sections[]`
...
| `numbering` | string  | 是       | 编号模式                      | 当前正式枚举为 `"auto"`/`"none"`/`"manual"` |
...
本节描述的是 `content.blocks[]` 中的 **文本块结构** 。
...
#### 14.7.3 当前版本正式支持的 `block.style` 枚举
...
| 字段名    | 类型   | 是否必须 | 含义                             | 说明                           |
| --------- | ------ | -------- | -------------------------------- | ------------------------------ |
| `_type` | string | 是       | 行内文本节点类型                 | 固定为 `"span"`              |
