...
为便于渲染器开发者理解，本协议按“**顶层字段顺序 + 每个字段的职责与引用关系**”组织：
...
- Portable Text 只负责承载 section 内部的富文本内容流。
...
Portable Text 中最常见的内容块类型是 `_type = "block"`。
...
本协议继续使用 `block` 作为正文富文本的基本单位，但对 `style` 和可出现位置做了进一步约束。

---

## 4.5 `style`：文本块的语义样式角色

`style` 用于表达该文本块在内容上的语义角色，而不是最终视觉样式。
...
本协议中，`style` 仍然表示“文本块角色”，但是否支持、如何渲染，以本协议正文块级文本样式定义为准。

渲染器应理解：

- `style` 表示语义，不直接等于最终视觉
- 字号多少、上下间距多少、是否换页，均由渲染器实现决定
- 对于本协议未列入正式支持范围的 `style`，应按回退规则处理，而不是自行推断为正式章节层级

特别需要注意的是：

- Portable Text 原生可用 `h1 / h2 / h3` 表达标题语义
- **本协议不使用 `h1 / h2 / h3` 表达正式章节层级**
- 本协议中的正式章节结构由顶层 `sections[]` 和 `subsection.section` 表达

也就是说：

- `block.style` 用于表达正文中的文本样式角色
- 正式章节树由协议字段表达，而不是由文本块样式推断

---

## 4.6 `span`：行内文本单位

`span` 是 `block.children[]` 中最基础的行内文本单位。

一个段落通常会拆成多个 `span`，这样才能只对局部文本施加不同标记。
...
本协议继续使用 `marks` 表达简单行内样式。
渲染器应按本协议允许的 decorator 集合实现。
...
本协议保留使用行内对象的能力，但是否正式支持某种行内对象类型，以本协议正文行内对象定义为准。
...
- **正文块级对象**：出现在正文内容流中
- **资源数据本体**：通常位于 `assets` 或 `datasets` 顶层字段中
...
`meta` 承载文档级元信息，不参与正文内容流。

### 7.3 当前版本建议支持的常见键

当前版本不强制展开完整字段表，但建议至少支持：
...
`assets` 是静态资源 registry。当前版本的最小结构为：
...
### 9.1 通用 registry 条目基型

`assets` 中的资源条目，推荐优先具备以下公共字段：
...
- `id`：全局唯一稳定标识
- `anchor`：锚点；省略时可回退为 `id`
- `label`：人类可读标签
- `meta`：附加元信息

### 9.2 `assets.images`：图片资源层

`assets.images` 存储的是**图片资源数据本体**，不是正文中的图片实例。

换句话说：

- `assets.images[]` 负责描述“这张图片资源是什么”
- 正文中的 `_type = "image"` 块负责描述“在这里插入哪张图片资源”
...
```json
{
  ...
  "width": 1200,
  "height": 800,
  "src": "https://example.com/system-overview.png",
  "imageSource": {
    "kind": "url",
...
- `alt`：应该，字符串。图片替代文本。
- `mimeType`：应该，字符串，例如 `image/png`、`image/jpeg`
- `checksum`：可以，字符串。建议使用 `sha256:<hex>`
- `width` / `height`：可以，整数。原始像素宽高
- `src`：兼容字段，字符串。为兼容旧版而保留
- `imageSource`：应该，对象。描述图片资源真实来源

#### 9.2.3 `imageSource.kind = "url"`
...
#### 9.2.4 `imageSource.kind = "embedded"`
...
- `data` 必须存在
- `encoding` 必须存在
- 当前版本正式支持的 `encoding` 只有 `"base64"`

#### 9.2.5 旧版 `src` 的兼容解释规则

若 `imageSource` 缺失，渲染器应按以下方式解释 `src`：

1. 若 `src` 以 `data:` 开头，则按 Data URI 处理
2. 否则按 URL / 文件路径处理
...
1. 若有 `imageSource`，优先按 `imageSource` 解析
2. 否则若有 `src`，按 `src` 的兼容逻辑解析
3. 否则视为图片资源缺失

### 9.3 `assets.logos` / `assets.backgrounds` / `assets.icons`

当前版本建议这三类资源与 `assets.images` 共享相同的基础资源结构与图片来源模式。

### 9.4 `assets.attachments`

`assets.attachments` 当前版本只保留 registry 位置，不定义详细字段合同。

---

## 10. `datasets`

`datasets` 是数据型资源 registry。当前版本的最小结构为：
...
### 10.1 `datasets.charts`：图表数据层

`datasets.charts[]` 存储的是**图表底层数据**，不是正文中的图表实例。

换句话说：

- `datasets.charts[]` 描述“图表数据是什么”
- 正文中的 `_type = "chart"` 块描述“在这里插入哪个图表数据”

#### 10.1.1 当前版本正式支持范围

当前版本只正式细化 `pie` 图。其他图表类型可以保留在 registry 中，但当前不构成稳定字段合同。
...
- `chartType`：必须，当前版本正式枚举只有 `"pie"`
- `valueUnit`：可以，例如 `percent`、`count`、`score`、`cny`
- `slices`：必须，数组
- `slices[].name`：必须
- `slices[].value`：必须
- `slices[].description`：可以
- `slices[].nameZh` / `nameEn` / `colorHint`：可以

#### 10.1.4 渲染器最低要求

- 必须能读取 `slices[].name` 与 `slices[].value`
- 若无颜色字段，颜色由渲染器自定
- 若有 `description`，可以用于图下注释或提示

### 10.2 `datasets.tables`：表格数据层

`datasets.tables[]` 存储的是**表格底层数据**，不是正文中的表格实例。

换句话说：

- `datasets.tables[]` 描述“表格数据是什么”
- 正文中的 `_type = "table"` 块描述“在这里插入哪个表格数据”
...
- `columns`：必须，数组。每列至少应包含 `key` 与 `header`
- `rows`：必须，数组。每行通常是一个对象，其键应与 `columns[].key` 对齐

#### 10.2.3 渲染器最低要求

- 应按 `columns` 顺序渲染表头与列顺序
- `rows` 中缺失的单元格应按空值处理
- 出现额外未声明列时，可以忽略并记录日志

### 10.3 `datasets.metrics`

当前版本只保留 registry 位置，不细化字段合同。
...
`bibliography[]` 存储的是**参考文献条目本体**。正文中如需引用参考文献，应通过行内 `citation_ref` 引用其 `id`。
...
`footnotes[]` 存储的是**脚注条目本体**。正文中如需引用脚注，应通过 `footnote_ref` 引用其 `id`。
...
`glossary[]` 存储的是**术语表条目本体**。正文中如需引用术语，应通过 `glossary_term` 引用其 `id`。
...
本节描述的是**正文实例层**，即这些对象如何出现在 `content.blocks[]` 中。
